...
get.Retrieve = function(self : remoteGet, ... : any)
    ...
    local connection; connection = self._rs:Connect(function(guid, ...)
...
remoteUtil.new = function(remoteName : string, remoteType : ("set" | "get")?)-- : typeof(Remote.ClientRemoteSignal.new(Instance.new("RemoteEvent"))) | typeof(Remote.RemoteSignal.new())
    assert(isClient and remoteFolder:FindFirstChild(remoteName) or not isClient, "[remote] You can only create remote on the server")
...
remoteUtil.get = function(remoteName)
