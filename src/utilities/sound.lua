...
soundUtil.makeGroup = function(soundGroupName : string)
    ...
    assert(not soundUtil.groups[soundGroupName], ("[sound] soundGroupName \"%s\" is already used"):format(soundGroupName))
...
if instance.firstChildWithCondition(SoundService, function(insc)
    if insc.ClassName == "Folder" and insc.Name == "sounds" then
        return true;
    end
end) or RunService:IsClient() then
