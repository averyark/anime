...
local d = {
    ...
    ["UDim2"] = function(udim2 : UDim2)
        return string.format("UDim2(%s, %s, %s, %s)", tostring(udim2.X.Scale), tostring(udim2.X.Offset), tostring(udim2.Y.Scale), tostring(udim2.Y.Offset));
    end,
...
function stringUtil.convertCS(s)
    --assert(t.string(s), "string expected")
    local classF = d[typeof(s)]
...
function stringUtil.iterateString<ag>(_s : string, callback : (_currentString : string, index : number, next: (...ag) -> (), ...ag) -> ())
    assert(t.string(_s), "string expected")
    local i = 1
    local _next; _next = function(...) : nil
        if i >= #_s then
            return;
        end
        i += 1
...
function stringUtil.shiftMatched(_s : string, _match : string)
...
function stringUtil.search<k, v>(input : string, _dictionary : {[string]: any}, algorithm : number?) : searchMeta
    ...
    local keywordSearch = function(_keyword : string, _input : string) : number -- decimal
        ...
        for i = 1, _kLen do
            for j = 1, _iLen, 1 do
                matrix[i][j] = math.min(matrix[i-1][j] + 1, matrix[i][j-1] + 1, matrix[i-1][j-1] + (_keyword:byte(i) == _input:byte(j) and 0 or 1))
            end
        end
...
-- returns readable table in a string
function stringUtil.tostringTable(_t)
    assert(t.table(_t), "expected table")
    local str = ""
    local manifest; manifest = function( _v, index : number)
        local spaces = stringUtil.makeSpace(index*5)
        for k, v in _v do
            if typeof(v) == "table" then
                str = str .. "\n" .. spaces .. "[" .. stringUtil.convertCS(k) .. "] = {"
                manifest(v, index+1)
                str = str .. "\n" .. spaces .. "}" .. if next(_v, k) then "," else ""
            else
                str = str .. "\n" .. spaces .. "[" .. stringUtil.convertCS(k) .. "] = " .. stringUtil.convertCS(v) .. if next(_v, k) then "," else ""
            end
        end
    end
