...
return {
		...
		for _ = 1, Amount or 15 do
			...
			if Raycast then
				...
				TweenService
					:Create(
						Rock,
						TweenInfo.new(0.4, Enum.EasingStyle.Linear, Enum.EasingDirection.In, -1),
						{ Orientation = Rock.Orientation + Vector3.new(360, 360, 360) }
					)
					:Play()
	...
	["Crater"] = function(Object, Radius, Size, Amount, AngleIndex)
		...
		for _ = 1, Amount or 18 do
			...
			Rock.CanCollide = false
			Rock.CanTouch = false
			Rock.CFrame = Object.CFrame
				* CFrame.fromEulerAnglesXYZ(0, math.rad(Angle), 0)
				* CFrame.new(0, -0.65, -Radius)
			Rock.Size = Vector3.new(Scale, Scale, Scale)
			...
			if Raycast then
				...
				TweenService
					:Create(
						Rock,
						TweenInfo.new(math.random(1.15, 1.35), Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
						{
